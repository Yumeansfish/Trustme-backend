...
class TrayIcon(QSystemTrayIcon):
    def __init__(
        ...
        self.setToolTip("trust-me" + (" (testing)" if testing else ""))
    ...
    def _build_rootmenu(self) -> None:
        ...
        if exitIcon.availableSizes():
            menu.addAction(exitIcon, "Quit trust-me", lambda: exit(self.manager))
        else:
            menu.addAction("Quit trust-me", lambda: exit(self.manager))
...
def run(manager: Manager, testing: bool = False) -> Any:
    ...
    if not QSystemTrayIcon.isSystemTrayAvailable():
        QMessageBox.critical(
            widget,
            "Systray",
            "I couldn't detect any system tray on this system. Either get one or run the trust-me modules from the console.",
        )
        sys.exit(1)

    if sys.platform == "darwin":
        icon = QIcon("icons:logo-128.png")
    else:
